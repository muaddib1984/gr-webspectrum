...
class RTLSDR_or_fake_signal_to_fft_to_zmq(gr.top_block):

    def __init__(self, fft_size=8192, frame_rate=30, frequency=100e6, gain=40.0, samp_rate=2.88e6):
        ...
        # Blocks
        ##################################################
        self.zeromq_pub_sink_0 = zeromq.pub_sink(gr.sizeof_float, fft_size, 'tcp://127.0.0.1:5001', 100, False, -1, '')
...
def argument_parser():
    ...
    parser.add_argument(
        "-r", "--frame-rate", dest="frame_rate", type=intx, default=30,
        help="Set frame rate [default=%(default)r]")
    parser.add_argument(
...
def main(top_block_cls=RTLSDR_or_fake_signal_to_fft_to_zmq, options=None):
    if options is None:
        options = argument_parser().parse_args()
    tb = top_block_cls(fft_size=options.fft_size, frame_rate=options.frame_rate, frequency=options.frequency, gain=options.gain, samp_rate=options.samp_rate)
